 ...
 */
export function ThemeSwitch({ storageKey, className }) {
    const { resolved, setTheme } = useTheme(storageKey);
    const goingTo = resolved === "dark" ? "light" : "dark";
    return (_jsx(IconButton, { label: goingTo === "dark" ? "Switch to dark mode" : "Switch to light mode", className: className, onClick: () => setTheme(goingTo), children: resolved === "dark" ? _jsx(IconSun, {}) : _jsx(IconMoon, {}) }));
}
