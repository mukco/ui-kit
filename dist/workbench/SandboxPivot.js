...
function toObjects(columns, rows, dims) {
...
export function SandboxPivot({ columns, rows, dimensionColumns }) {
    const dims = useMemo(() => new Set([...DEFAULT_DIMENSIONS, ...(dimensionColumns ?? [])]), [dimensionColumns]);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const data = useMemo(() => toObjects(columns, rows, dims), [columns, rows, dims]);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return (_jsxs("div", { className: "ui-sb-pivot", children: [_jsxs("div", { className: "ui-sb-tips", children: [_jsx("span", { className: "ui-sb-tipslabel", children: "Quick starts" }), TIPS.map((t) => (_jsxs("span", { className: "ui-sb-tip", children: [_jsxs("strong", { children: [t.label, ":"] }), " ", t.hint] }, t.label))), _jsx("span", { className: "ui-sb-note ui-sb-tipsright", children: "Drag pills into Rows / Cols zones \u00B7 pick aggregation" })] }), _jsx("div", { className: "ui-sb-pivotbody", children: _jsx(PivotTableUI, { data: data, aggregators: AGGREGATORS, renderers: RENDERERS, onChange: setPivotState, ...pivotState }) })] }));
}
