...
export const normalizeName = (s) => (s ?? "").toString().normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase();
/**
 ...
 */
export const matchKey = (s) => normalizeName(s)
    .replace(/[^a-z ]/g, " ")
...
export function extractCandidates(text) {
    if (!text)
        return [];
    const raw = (String(text).match(NAME_RE) || []).map((n) => n.replace(/'s$/, ""));
 ...
 */
export function resolveKnownPlayers(text, players) {
    const map = {};
    if (!text || !Array.isArray(players))
        return map;
    ...
    for (const p of players) {
        if (p?.name == null || p?.id == null)
            continue;
        if (body.includes(p.name) || tKey.includes(matchKey(p.name)))
            map[p.name] = p.id;
    }
    return map;
}
